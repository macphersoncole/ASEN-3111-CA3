...
% Date: 27th Feb 2021
    ...
    q_inf = (1/2)*rho*(V_inf)^2; % free-stream dynamic pressure [Pa]
    alpha = deg2rad(alpha); % convert AoA from degrees to radians [rad]
    ...
    %% Calculate Gamma
    Gamma = sum(2*pi*V_inf*gamma.*ds); % circulation
    
    %% Calculate sectional lift
    lift = rho*V_inf*Gamma; % sectional lift
    
    %% Calculate sectional cefficient of lift
    c_l = lift/(q_inf*c); % sectional coefficient of lift
